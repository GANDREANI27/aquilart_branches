...
// ─────────────────────────────────────────────
// LIGHTBOX ORGANIGRAMME
// ─────────────────────────────────────────────
function openOrga() {
  document.getElementById('orga-lightbox').classList.add('active');
  document.body.style.overflow = 'hidden';
}

function closeOrga(e) {
  if (e && e.target !== document.getElementById('orga-lightbox') && !e.target.closest('.orga-lightbox-close')) return;
  document.getElementById('orga-lightbox').classList.remove('active');
  document.body.style.overflow = '';
}

document.addEventListener('keydown', e => {
  if (e.key === 'Escape') closeOrga({ target: document.getElementById('orga-lightbox') });
});
